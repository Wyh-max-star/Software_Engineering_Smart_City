...
def apply_selection(base, sel):
    ...
    for cat, itype in STREET_CATS.items():
        if sel.get(cat):
            ...
            results.append(
                (CAT_TITLE.get(cat, cat), apply_street_asset(itype, sel[cat])))
    for cat, mtype in MATERIAL_CATS.items():
        if sel.get(cat):
            enter_edit_select_all(base)
            results.append(
                (CAT_TITLE.get(cat, cat), apply_road_material(sel[cat], mtype)))
    if bpy.context.object and bpy.context.object.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
    return results
...
def apply_scene_dimensions(scene_cfg):
    ...
    for name, attr, gen_op, pre_clear_op, set_enable in SCENE_DIMS:
        _apply_one_dimension(name, scene_cfg.get(
            name), attr, gen_op, pre_clear_op, set_enable, results)
    return results
...
def _ensure_weather_particles(weather_type):
    ...
    # 添加粒子系统
    mod = emitter.modifiers.new(
        name="WeatherParticles", type='PARTICLE_SYSTEM')
...
def apply_weather(weather_mode):
    ...
    if world and world.node_tree:
        ...
        if weather_mode == "sunny":
            ...
            if color_input:
                color_input.default_value = (0.6, 0.8, 1.0, 1.0)
            _log("  天气: 晴朗")
        elif weather_mode == "cloudy":
            if strength_input:
                strength_input.default_value = 0.35
            if color_input:
                color_input.default_value = (0.5, 0.5, 0.55, 1.0)
            _log("  天气: 阴天/天色变暗")
        elif weather_mode == "rainy":
            if strength_input:
                strength_input.default_value = 0.2
            if color_input:
                color_input.default_value = (0.35, 0.35, 0.4, 1.0)
            _log("  天气: 雨天")
            _ensure_weather_particles("rainy")
        elif weather_mode == "night":
            if strength_input:
                strength_input.default_value = 0.02
            if color_input:
                color_input.default_value = (0.02, 0.02, 0.08, 1.0)
            _log("  天气: 夜晚")
        elif weather_mode == "foggy":
            if strength_input:
                strength_input.default_value = 0.3
            if color_input:
                color_input.default_value = (0.55, 0.55, 0.6, 1.0)
            _log("  天气: 起雾")
            _set_eevee_fog(True)
        elif weather_mode == "sunset":
            if strength_input:
                strength_input.default_value = 0.6
            if color_input:
                color_input.default_value = (1.0, 0.6, 0.2, 1.0)
            _log("  天气: 黄昏")
        elif weather_mode == "snowy":
            if strength_input:
                strength_input.default_value = 0.25
            if color_input:
                color_input.default_value = (0.5, 0.55, 0.6, 1.0)
            _log("  天气: 下雪")
...
def template_enum_items(self, context):
    ...
    for t in template_core.load_templates():
        tid = str(t.get("id"))
        _enum_cache.append(
            (tid, f"{tid} · {t.get('name', tid)}", t.get("_comment", "")))
    if not _enum_cache:
...
class ICITY_OT_ApplyNaturalLanguage(Operator):
    ...
    def execute(self, context):
        ...
        if not template_core._has_content(sel):
            self.report(
                {'WARNING'}, f"[{engine}] 没听懂。试试含 棕榈/金黄/脏/车水马龙/有山有湖/很多人 等词")
            return {'CANCELLED'}
        ...
        self.report(
            {'INFO'}, f"[{engine}] 已应用: {_fmt_results(results)} ✓ 关Proxy+材质预览看效果")
        return {'FINISHED'}
...
class ICITY_PT_TemplatePanel(Panel):
    ...
    def draw(self, context):
        ...
        if tpl:
            ...
            box.label(text="树: " + pl.get("tree", "-") +
                      " / 椅: " + pl.get("bench", "-"))
            box.label(text="路面: " + pl.get("road_material", "-"))
            tr = sc.get("traffic", {})
            pe = sc.get("pedestrian", {})
            ec = sc.get("ecology", {})
            eco = ec.get("ecology_plot_mode", "无") if ec.get(
                "enable", True) else "无"
            box.label(
                text=f"车{tr.get('car_count', '-')}/出租{tr.get('taxi_count', '-')}/公交{tr.get('bus_count', '-')} · 行人{pe.get('walker_count', '-')} · 生态{eco}")
        ...
        col.scale_y = 0.8
        engine = "DeepSeek" if (
            LLM_ENABLED and template_core.get_api_key()) else "规则(未配key)"
...
def register() -> None:
    ...
    bpy.types.Scene.icity_template_settings = PointerProperty(
        type=ICITY_TemplateSettings)
